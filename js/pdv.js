...
async function criarPagamentoPix(comandaId, valor) {
  try {
    ...
    const { txid, qrcode, pixCopiaECola, qrcodeImagem, ticketUrl, ambienteTeste, pixSandboxReal, pixSimulacaoLocal, validadeEm, expiracaoSegundos } = result.data;
    ...
    iniciarVerificacaoPeriodicaPix(comandaId, txid, result.data.pixSandboxReal ? 2000 : 10000);
...
async function simularConfirmacaoPixLocal(comandaId, txid) {
  try {
    ...
    const fn = firebase.functions().httpsCallable('simularConfirmacaoPixTeste');
    await fn({ comandaId, txid });
    fecharLoading();
